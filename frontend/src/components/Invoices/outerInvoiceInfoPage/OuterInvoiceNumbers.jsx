...
export const OuterInvoiceNumbers = ({ data }) => {
    ...
    return (
        <div className="mt-8 sm:mt-5 w-[80%] mx-auto flex flex-col gap-5">
            <dl className="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-3 gap-4 sm:grid-cols-2">
            ...
            <dl className="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-3 gap-4 sm:grid-cols-2">
                ...
                <GridItem notCash={true} label="Total Discount Percent" value={`${data ? ((data.extra_discount ) * 100).toFixed(1) : 0}%`} />
                <GridItem label="Total Cost" value={data ? data.total_cost ? data.total_cost.toFixed(1) : '0' : ''} />
            </dl>
