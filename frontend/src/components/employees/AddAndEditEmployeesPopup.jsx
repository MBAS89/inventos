...
import { useAddEmployeeMutation, useEditEmployeeMutation, useReadEmployeeQuery } from '../../features/api/employees/employeeApiSlice'
...
export const AddAndEditEmpolyeesPopup = ({ setOpenPopup, editMode, selected, setEditMode }) => {
    ...
    const [editEmployee, {isLoading:isEditLoading, error:editError}] = useEditEmployeeMutation()
    ...
    <section className="overflow-auto bg-white left-[32%] top-[7%] h-[52rem] w-[45rem] border-gray-300 border-solid border-[1px] absolute rounded-lg shadow-2xl">
        ...
        <div className='flex items-center w-[70%] mx-auto mt-3 gap-5'>
            ...
            <button type='button' onClick={() => setWithContract(true)} className={`inline-block ${withContract ? 'text-white bg-[#50B426]' : 'text-[#50B426] bg-white' } rounded border w-full border-[#50B426] px-12 py-4 text-sm font-medium active:bg-green-500 text-[1.3rem]`}>With Contract</button>
        </div>
        <form onSubmit={editMode ? handleEditEmployee : handleAddEmployee} className='flex flex-col gap-8 w-[70%] mx-auto mt-5 mb-10'>
            ...
            <button type='submit' disabled={isEmployeeLoading} className="flex justify-center rounded border w-full border-[#50B426] px-12 py-4 text-sm font-medium text-[#50B426] active:bg-green-500 active:text-white text-[1.3rem]">
                {isEmployeeLoading && <BiLoaderCircle className='text-[1.4rem] animate-spin'/>} {editMode ? withContract ? 'Edit Employee With Contract' :'Edit Employee' : withContract ? 'Add Employee With Contract' :'Add Employee'}
            </button>
        </form>
    </section>
  )
