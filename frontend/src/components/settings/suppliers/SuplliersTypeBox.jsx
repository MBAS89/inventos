...
//redux
import { useEditSuppliersTypeMutation, useRemoveSuppliersTypeMutation } from '../../../features/api/suppliers/suppliersTypesApiSlice';

export const SuplliersTypeBox = ({typeData}) => {

    const [typeName, setTypeName] = useState(typeData.type_name)

    const [removeSuppliersType] = useRemoveSuppliersTypeMutation()

    const handleDeleteType = async () => {
        try {
            const payload = {
                supplierId:typeData.id,
            }

            const res = await removeSuppliersType(payload).unwrap()
    ...
    const [editSuppliersType] = useEditSuppliersTypeMutation()
    ...
    const handleSubmitEdit = async () => {
        ...
        try {
            ...
            const res = await editSuppliersType(payload).unwrap()
            setEditMode(false)
            toast.success(res.message)
        } catch (error) {
            toast.error(error.data.error)
