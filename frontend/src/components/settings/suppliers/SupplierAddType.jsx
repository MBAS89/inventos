...
import { useAddSuppliersTypeMutation } from '../../../features/api/suppliers/suppliersTypesApiSlice'
import { toast } from 'react-toastify'

import { BiLoaderCircle } from "react-icons/bi";
export const SupplierAddType = () => {

    const [typeName, setTypeName] = useState('')

    const [addSuppliersType, { isLoading }] = useAddSuppliersTypeMutation()

    const handleAddSupplierType = async (e) => {
        e.preventDefault()
        ...
        try {
            const res = await addSuppliersType(payload).unwrap()
    ...
    return (
        <div className='mt-2 p-4'>
            <form onSubmit={handleAddSupplierType} className='flex gap-4 bg-white p-5 rounded-lg w-[40rem] ml-10'>
