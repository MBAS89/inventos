...
import { useReadBrandQuery } from '../../features/api/inventory/brandApiSlice'

export const BrandProfile = () => {
    const { brandId } = useParams();

    const [currentPage, setCurrentPage] = useState(1)
    const { data:brand, isLoading } = useReadBrandQuery({brandId: brandId, products:true, page:currentPage }, 'readBrand')
    ...
    <div className='w-[80%] mx-auto'>
        <div className='h-[15rem] mt-10 flex gap-5'>
            <div className='w-[60%] bg-white h-full rounded-md border-gray-200 border-2 p-4'>
                <h2 className='font-bold text-[1.3rem] pl-4 mb-2'>Brand Information</h2>
                <div className='flex items-center gap-10 h-[90%] pr-16 pl-10'>
                    {isLoading ? (
                        ...
                        <div className=' bg-[#50B426] rounded-full w-[128px] h-[128px] p-1'>
                            <img  width="128" height="128"  src={brand.brand.image} className='rounded-full' alt={brand.brand.name} />
                        </div>
                    )}
                    <div className='flex gap-10'>
                        <div className='flex flex-col gap-5'>
                            <div>
                                <h5 className='text-gray-400'>Brand Name</h5>
                                {isLoading ? (
                                    <div className='bg-slate-500 animate-pulse h-[20px] w-[140px] rounded-lg'></div>
                                ) : (
                                    <p className='font-bold'>{brand.brand.name}</p>
                                )}
                            </div>  
                        </div>
                    </div>
